...
export const createTeams=teamInfos=>teamInfos.map(info=>{
  ...
  const usedIds=new Set(initialLines.flatMap(l=>l.players).map(p=>p.identity.id));
  const reserveProfiles=getTeamProfiles(info.id).filter(p=>!usedIds.has(p.id));
  const reservePlayers=reserveProfiles.map(p=>createSkater(info,p.identity.firstName,p.identity.lastName,p.position,seasonId,p));
  const roster=[...initialLines.flatMap(l=>l.players),...reservePlayers];
