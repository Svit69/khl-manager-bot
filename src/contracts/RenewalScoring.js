...
const topPlayerCompetitiveOutlookScore = (player, context, reasons) => {
  if ((player?.ovr || 0) < TOP_PLAYER_OVR_THRESHOLD) return 0;
  ...
  reasons.push({ text: label, value: score });
  return score;
};
...
const repeatedBadOfferScore = (context, reasons) => {
  const badOfferCount = Number(context?.badOfferCount) || 0;
