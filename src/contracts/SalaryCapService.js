...
export class SalaryCapService {
  ...
  assessTrade({ contracts, userTeamId, aiTeamId, givePlayerIds, receivePlayerIds, seasonLabel }) {
    const playerIds = [...new Set([...(givePlayerIds || []), ...(receivePlayerIds || [])])];
    ...
    return [
      this.#assessTradeTeam({ contracts, teamId: userTeamId, outgoingIds: givePlayerIds, incomingIds: receivePlayerIds, seasons }),
      this.#assessTradeTeam({ contracts, teamId: aiTeamId, outgoingIds: receivePlayerIds, incomingIds: givePlayerIds, seasons }),
  ...
  #buildOfferSeasons(startSeason, years = 1) {
    let season = startSeason;
    return Array.from({ length: Math.max(1, Number(years) || 1) }, () => [season, season = formatNextSeason(season)][0]);
  }
