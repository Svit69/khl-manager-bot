...
export class Renderer{
  ...
  renderFantasyDraft(draft,team){
    ...
    const nextMatch=this.#nextMatchCard.render(currentMatch,panelData?.standings,currentDateLabel);
    this.#calEl.innerHTML=`<div class="calendar-shell"><section class="calendar-top-block"><div class="calendar-date-row"><div><div class="calendar-date-label">${String(currentDateLabel).toUpperCase()}</div><div class="calendar-state-label">${String(text).toUpperCase()}</div></div><button id="playBtn" class="btn calendar-next-btn" ${isLocked?"disabled":""}>ДАЛЬШЕ</button></div>${tabButtons}</section><section class="calendar-content-block"><div class="calendar-panel-list${panelClass}">${tableHeader}<div class="calendar-panel-scroll${panelClass}">${tableBody}</div></div></section><section class="calendar-split-block"><div>${bestTeamPlayer}</div><div>${nextMatch}</div></section><section class="calendar-split-block"><div></div><div></div></section></div>`;
  }
