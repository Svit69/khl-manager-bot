...
export class Renderer{
  ...
  renderFantasyDraft(draft,team){
    ...
    const salaryCap=draft.salaryCap?{
      ...
      selectedDisplaySalaryRub:previewPlayer?Number((draft.salaryCap.displaySalaryByPlayerId||draft.salaryCap.salaryByPlayerId)?.[previewPlayer.id])||0:0,
    ...
    const cards=draft.availablePlayers.map(player=>{
      ...
      const salaryRub=(salaryCap?.displaySalaryByPlayerId||salaryCap?.salaryByPlayerId)?.[player.id]||0;
    ...
    const previewSalary=salaryCap&&previewPlayer?` • ЗП ${formatMillions(salaryCap.selectedDisplaySalaryRub)} млн`:"";
    ...
    const scheduleRows=(panelData?.schedule||[]).map(row=>{
      if(row.isRestDay){
        return `<div class="calendar-schedule-row${row.isCurrent?" current":""}"><span class="day">${row.shortDateLabel||row.day}</span><span class="teams"><strong>${row.isCurrent?"\u0414\u0435\u043d\u044c \u043e\u0442\u0434\u044b\u0445\u0430 (\u0442\u0435\u043a\u0443\u0449\u0438\u0439)":"\u0414\u0435\u043d\u044c \u043e\u0442\u0434\u044b\u0445\u0430"}</strong></span><span class="res">${row.isPlayed?"\u2713":"\u2014"}</span></div>`;
      }
      ...
      return `<div class="calendar-schedule-row${row.isCurrent?" current":""}${row.isMyMatch?" mine":""}"><span class="day">${row.shortDateLabel||row.day}</span><span class="teams"><strong>${previewText}</strong><small>${secondaryText}</small></span><span class="res">${primaryResult}</span></div>`;
