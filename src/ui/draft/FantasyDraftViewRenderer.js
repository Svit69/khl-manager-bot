...
export const renderFantasyDraftView = (draft, team) => {
  ...
  return `<div class="draft-screen"><div class="draft-top">${renderHeader(draft, team)}${renderDraftCapSummary(salaryCap)}<div class="draft-order-strip">${order}</div></div><div class="draft-layout"><section class="draft-left"><div class="draft-card"><div class="draft-card-head"><h2>Доступные игроки</h2><div class="muted">${draft.availablePlayers.length} в пуле</div></div>${renderControls(draft)}${action}<div class="draft-list">${renderDraftPlayerList(draft, salaryCap) || '<div class="muted">Нет игроков</div>'}</div></div></section><aside class="draft-right"><div class="draft-card"><div class="draft-card-head"><h2>Просмотр игрока</h2><div class="muted">Скиллы и контракт</div></div>${renderPreview(preview, salaryCap)}</div><div class="draft-card"><div class="draft-card-head"><h2>Ваш драфт-борд</h2><div class="muted">${team.name}</div></div>${renderDraftNeedsGrid(draft.userRosterByPosition || {})}${renderDraftRosterPanel(draft.userRosterByPosition || {})}</div><div class="draft-card"><div class="draft-card-head"><h2>Команды</h2><div class="muted">${draft.rounds} раундов • змейка</div></div><div class="draft-team-list">${renderTeams(draft, team)}</div></div></aside></div></div>`;
};
